...
from functions.decoders import wiener_decoder
...
# set Wiener filter parameters
timesteps = 15 # number of timesteps (current+previous)
regular = 'l2' # regularisation type
alpha = 0 # regularisation constant
params = {'timesteps':timesteps, 'regular': regular, 'alpha':alpha}
...
for i in range(num_fold):    
    ...
    model = wiener_decoder() # instantiate model
    start = timer.time()
...
result_filename = result_folder+file_name+'_wf_'+feature+'_'+str(int(wdw_time*1e3))+'ms.h5'
